...
final class TestFilmsNavigator: FilmsNavigator {
    func toFilms() { }
    func toFilm(_ film: Film) { }
}

final class TestFilmsApiProvider: FilmsApiProvider {
...
"""
    ...
    static var films: [Film] {
        guard let data = TestFilmsApiProvider.filmsJson.data(using: .utf8) else { return [] }
        return try! JSONDecoder().decode([Film].self, from: data)
    }
    
    func films() -> Observable<[Film]> {
        return Observable.just(TestFilmsApiProvider.films)
    }
}
